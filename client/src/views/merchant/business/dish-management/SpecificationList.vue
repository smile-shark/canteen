  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(`确定要删除【${row.name}】吗?`, "提示", {
        ...
        .then(() => {
          api.cuisineComponent.delete(row.cuisineComponentId).then((res) => {
            if (res.data.code == 200) {
    ...
    handleStatusChange(row) {
      ...
      this.$message.success(
        `规格【${row.name}】已${row.status === "1" ? "启用" : "禁用"}`
      );
    },
    ...
    saveCategory() {
      ...
      api.cuisineComponent.add(this.form).then((res) => {
        if (res.data.code == 200) {
